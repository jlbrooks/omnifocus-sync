...
# Compatibility: newer cryptography versions don't need/have default_backend()
try:
...
# backend already set above for compatibility
...
'''
        ...
        if hasattr(metadata_blob_or_fp, 'read'):
            if hasattr(plistlib, 'load'): # Py3
                try:
                    ...
                    metadata_blob_or_fp.seek(0)
                    metadata = plistlib.load(metadata_blob_or_fp)
            else:
            ...
            if hasattr(plistlib, 'loads'): # Py3
                try:
                    ...
                    metadata = plistlib.loads(metadata_blob_or_fp)
            else:
    ...
    def use_passphrase(self, metadata, passphrase):
        ...
        # Handle both old plistlib.Data and new bytes type
...
def decrypt_directory(indir, outdir, re_encrypt=False):
    ...
    key_obj = encryptionMetadata.get('key')
    key_data = key_obj.data if hasattr(key_obj, 'data') else key_obj
    docKey = DocumentKey( key_data, unwrapping_key=metadataKey )
